...
def render_overall_market_sentiment(NSE_INSTRUMENTS=None):
    ...
    # ═══════════════════════════════════════════════════════════════════
    # ADDITIONAL BIAS METRICS (ATM STRIKE, ROTATION, PCR)
    # ═══════════════════════════════════════════════════════════════════

    st.markdown("#### 🎯 Additional Bias Metrics")

    # Create columns for additional bias displays
    bias_cols = st.columns(3)

    # ─────────────────────────────────────────────────────────────────
    # 1. ATM STRIKE VERDICT
    # ─────────────────────────────────────────────────────────────────
    with bias_cols[0]:
        if 'nifty_option_screener_data' in st.session_state:
            atm_bias = st.session_state.nifty_option_screener_data.get('atm_bias')
            if atm_bias:
                ...
                if 'BULLISH' in str(atm_verdict).upper():
                    verdict_color = '#00ff88'
                    verdict_icon = '🐂'
                elif 'BEARISH' in str(atm_verdict).upper():
                ...
                # Get PCR sentiment if available (for additional context)
                pcr_sentiment = ""
                if 'PCR Analysis' in result.get('sources', {}):
                    pcr_data = result['sources']['PCR Analysis']
                    pcr_bias = pcr_data.get('bias', 'NEUTRAL')
                    if pcr_bias != atm_verdict:
                        pcr_sentiment = f"<div style='font-size: 0.8rem; color: #888; margin-top: 5px;'>PCR falling sharply (bearish buildup)</div>"

                st.markdown(f"""
                <div style='background: linear-gradient(135deg, #2d2d2d 0%, #1e1e1e 100%);
                            padding: 15px; border-radius: 10px; border-left: 4px solid {verdict_color};'>
                    <div style='font-size: 0.9rem; color: #888; margin-bottom: 8px;'>🎯 ATM STRIKE VERDICT</div>
                    <div style='font-size: 1.5rem; color: {verdict_color}; font-weight: bold; margin-bottom: 5px;'>
                        {verdict_icon} {atm_verdict}
                    </div>
                    <div style='font-size: 1rem; color: #ffffff; margin-top: 5px;'>
                        Strike: {atm_strike} | Score: {atm_score:+.2f}
                    </div>
                    {pcr_sentiment}
                    <div style='font-size: 0.8rem; color: #66b3ff; margin-top: 10px; padding-top: 10px; border-top: 1px solid #3d3d3d;'>
                        SENTIMENT: {atm_verdict}
                    </div>
                </div>
                """, unsafe_allow_html=True)
            else:
                st.info("⏳ ATM Strike data loading...")
        else:
            st.info("⏳ ATM Strike data loading...")
    ...
    with bias_cols[1]:
        if 'enhanced_market_data' in st.session_state:
            ...
            if market_data and market_data.get('success'):
                ...
                # Determine color based on rotation bias
                if 'STRONG BULLISH' in str(rotation_bias).upper():
                    rotation_color = '#00ff88'
                    rotation_icon = '🚀'
                elif 'BULLISH' in str(rotation_bias).upper():
                    rotation_color = '#00cc66'
                    rotation_icon = '🐂'
                elif 'STRONG BEARISH' in str(rotation_bias).upper():
                    rotation_color = '#ff4444'
                    rotation_icon = '📉'
                elif 'BEARISH' in str(rotation_bias).upper():
                    rotation_color = '#cc0000'
                    rotation_icon = '🐻'
                else:
                    rotation_color = '#ffa500'
                    rotation_icon = '⚖️'

                st.markdown(f"""
                <div style='background: linear-gradient(135deg, #2d2d2d 0%, #1e1e1e 100%);
                            padding: 15px; border-radius: 10px; border-left: 4px solid {rotation_color};'>
                    <div style='font-size: 0.9rem; color: #888; margin-bottom: 8px;'>🏢 ROTATION BIAS</div>
                    <div style='font-size: 1.5rem; color: {rotation_color}; font-weight: bold; margin-bottom: 5px;'>
                        {rotation_icon} {rotation_bias}
                    </div>
                    <div style='font-size: 1rem; color: #ffffff; margin-top: 5px;'>
                        Score: {rotation_score:+d}
                    </div>
                    <div style='font-size: 0.8rem; color: #66b3ff; margin-top: 10px; padding-top: 10px; border-top: 1px solid #3d3d3d;'>
                        Sector momentum: {rotation_bias}
                    </div>
                </div>
                """, unsafe_allow_html=True)
            else:
                st.info("⏳ Rotation bias data loading...")
        else:
            st.info("⏳ Rotation bias data loading...")
    ...
    with bias_cols[2]:
        if 'PCR Analysis' in result.get('sources', {}):
            ...
            # Determine color based on PCR bias
            if 'BULLISH' in str(pcr_bias).upper():
                pcr_color = '#00ff88'
                pcr_icon = '🐂'
            elif 'BEARISH' in str(pcr_bias).upper():
                pcr_color = '#ff4444'
                pcr_icon = '🐻'
            else:
                pcr_color = '#ffa500'
            ...
            # Get NIFTY PCR if available
            nifty_pcr = "N/A"
            for detail in pcr_details:
                if detail.get('Instrument') == 'NIFTY':
                    nifty_pcr = detail.get('PCR (OI)', 'N/A')
                    break

            st.markdown(f"""
            <div style='background: linear-gradient(135deg, #2d2d2d 0%, #1e1e1e 100%);
                        padding: 15px; border-radius: 10px; border-left: 4px solid {pcr_color};'>
                <div style='font-size: 0.9rem; color: #888; margin-bottom: 8px;'>📊 PCR ANALYSIS</div>
                <div style='font-size: 1.5rem; color: {pcr_color}; font-weight: bold; margin-bottom: 5px;'>
                    {pcr_icon} {pcr_bias}
                </div>
                <div style='font-size: 1rem; color: #ffffff; margin-top: 5px;'>
                    NIFTY PCR: {nifty_pcr} | Score: {pcr_score:+.1f}
                </div>
                <div style='font-size: 0.8rem; color: #66b3ff; margin-top: 10px; padding-top: 10px; border-top: 1px solid #3d3d3d;'>
                    Put-Call Ratio Sentiment: {pcr_bias}
                </div>
            </div>
            """, unsafe_allow_html=True)
        else:
            st.info("⏳ PCR analysis data loading...")
